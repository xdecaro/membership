...
def validate():
 ...
 source=require('component/admin/src/Service/AnalyticsSourceService.php','assertAuthorised','#__decaromembership_members','membership.members.total','membership.expiring')
 ...
 require('package/script.php',"$type!=='install'","'xdecaroanalytics'","'task'",'decaromembership')
 if not (ROOT/f'component/admin/sql/updates/mysql/{VERSION}.sql').is_file(): fail('schema marker missing')
 # Preserve the legacy table; this release changes new reminder behavior without deleting existing data.
